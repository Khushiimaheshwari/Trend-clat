...
const AdminHome = () => {
  ...
  const adminCategories = [
    { name: "Dashboard", icon: <BarChart size={24} />, count: "Overview", to: "/admin_dashboard" },
    { name: "Products", icon: <Package size={24} />, count: "416 items", to: "/admin_products" },
    { name: "Customers", icon: <Users size={24} />, count: "3.2k users", to: "/admin_customers" },
    { name: "Orders", icon: <ShoppingBag size={24} />, count: "184 pending", to: "/admin_orders" }
  ];
  ...
  // Admin gradient component similar to Home_Gradient
  const AdminGradient = () => (
  ...
  return (
    <div className="flex flex-col min-h-screen z-0 -mt-12 bg-slate-950 text-gray-100 relative">
      ...
      <section className="flex items-center justify-center pt-32 pb-16 bg-slate-950 text-white relative z-10">
        <div className="text-center items-center max-w-5xl p-6">
          ...
          <p className="text-lg md:text-xl px-6 text-lime-100 mb-8">
            <em>
              Manage your products, monitor sales, and optimize your online store with our comprehensive admin tools.
            </em>
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <button 
              onClick={() => document.getElementById('admin_controls')?.scrollIntoView({ behavior: 'smooth' })}
              className="bg-lime-100 hover:bg-lime-200 hover:scale-105 cursor-pointer text-black font-bold px-6 py-3 rounded-md transition duration-300">
              Manage Products
            </button>
     ...
     {/* Hero Slider */}
<div id='slider' className="relative text-white mb-15 py-16 md:py-32 overflow-hidden transition-all duration-500"
    ...
    <div className="max-w-xl">
      <div className="flex items-center gap-3 mb-2">
        <div className="bg-white/20 p-2 rounded-full">
          {heroImages[heroImageIndex].icon}
        </div>
      ...
      <div className="mt-4">
        <h3 className="text-4xl md:text-6xl font-bold mb-2">
          {heroImages[heroImageIndex].value}
        </h3>
        <p className={`text-lg font-medium flex items-center gap-1 ${
          heroImages[heroImageIndex].isPositive ? 'text-green-400' : 'text-red-400'
        }`}>
          <span>{heroImages[heroImageIndex].change}</span>
          <span className="text-sm">since last period</span>
        </p>
      </div>
    </div>
  </div>
  ...
  <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-2">
    {heroImages.map((_, index) => (
      ...
      {/* Admin Categories */}
      <section id='admin_controls' className="py-16 bg-slate-950 relative z-10">
        <div className="container mx-auto px-4 mt-10">
          <h2 className="text-3xl font-bold mb-12 text-center text-white">ADMIN CONTROLS</h2>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {adminCategories.map((category, index) => (
              <div
                key={index}
                onClick={() => handleCategoryClick(category.to)}
                className="cursor-pointer group bg-slate-800 p-6 rounded-lg hover:bg-slate-700 transition duration-300 transform hover:scale-105 hover:shadow-lg flex flex-col items-center"
              >
          ...
          <div className="py-12 px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              <div className="bg-slate-700 rounded-lg p-6 text-center">
                <div className="text-4xl font-bold mb-2 text-teal-200">416</div>
                <div className="text-gray-300">Total Products</div>
              </div>
              <div className="bg-slate-700 rounded-lg p-6 text-center">
                <div className="text-4xl font-bold mb-2 text-rose-400">184</div>
                <div className="text-gray-300">Pending Orders</div>
              </div>
              <div className="bg-slate-700 rounded-lg p-6 text-center">
                <div className="text-4xl font-bold mb-2 text-purple-300">4,287</div>
                <div className="text-gray-300">Active Customers</div>
              </div>
              <div className="bg-slate-700 rounded-lg p-6 text-center">
                <div className="text-4xl font-bold mb-2 text-yellow-300">₹345,897</div>
